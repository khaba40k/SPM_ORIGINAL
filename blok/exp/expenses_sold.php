...
<script>
    ...
    function addRow($for = 'in') {
        ...
        $.get('blok/exp/expenses_sold.php', 'newRow=' + $lastId + '&for=' + $for, function(result)
        {
        ...
        $(".expenses_table").on("change", "select", function() {
            ...
            if (row.substring(0, 2) == 's_') {//Якщо змінено назву послуги
                ...
                $.get('blok/exp/expenses_sold.php', 'row_id=' + row.substring(2) + '&typeFor=' + serv_id, function (result) {
                    td_type.innerHTML = result;
                });

                $.get('blok/exp/expenses_sold.php', 'row_id=' + row.substring(2) + '&count=' + serv_id, function (result) {
                    td_count.innerHTML = result;
                });

                $.get('blok/exp/expenses_sold.php', 'row_id=' + row.substring(2) + '&color_for=' + serv_id + '&type=1', function (result) {
                    td_color.innerHTML = result;
                });

                    <?php
                    if ($_GET['for'] == 'out') {
                        echo "$.get('blok/exp/expenses_sold.php', 'row_id=' + row.substring(2) +
                             '&price_for=' + serv_id +
                ...
                $.get('blok/exp/expenses_sold.php', 'row_id=' + row_type[1] +
                    ...
                    <?php
                    if ($_GET['for'] == 'out') {
                        echo "$.get('blok/exp/expenses_sold.php', 'row_id=' + row_type[1] +
        ...
        if ($(count_cell).val() < 0) {
            ...
            if ($(price_cell).val() == 0) {
                $(price_cell).val('');
                         ...
                         <?php
                    if ($_GET['for'] == 'out') {
                        echo "$.get('blok/exp/expenses_sold.php', 'row_id=' + idrow +
