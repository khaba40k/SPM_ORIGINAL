<script>
    ...
    function insertTable($in = '') {
              ...
              $.ajax({
                    url: 'blok/exp/expenses_sold.php',
                    method: 'GET',
                  dataType: 'html',
                  data: 'for=out' + $in,
...
$Z_DATA->DATE_IN = date("Y-m-d\TH:i");
...
if($TYPE_Z == 'def' || $TYPE_Z == 'def0'){
    ...
    if($TYPE_Z != 'def0'){
        $div[] = new HTEL('div', [
            ...
            new HTEL('input !=datin *=datetime-local ?=date_in #=[3] [r]')
        ]);
...
if (!is_null($Z_DATA->DATE_OUT)) {
    ...
    $field1(new HTEL('div', [
        ...
        new HTEL('input !=datout *=datetime-local ?=date_out #=[5]')
    ]));
}
