...
$next_day_date = date('Y-m-d', strtotime($_GET['do'] . ' +1 day'));
...
foreach ($arr_serv_ids as $i) {
    ...
    foreach ($type_variants as $t){
        ...
        $query = 'SELECT * FROM (SELECT string_name, string_type, SUM(_count) as _cnt_in, SUM(_cost) AS _sum_in FROM get_view_in WHERE _id = ' . $i . ' AND _type = ' . $t . ' AND _count > 0 AND _date >= "' . $_GET['ot'] . '" AND _date < "' . $next_day_date . '") _in
                 INNER JOIN (SELECT SUM(_count)*-1 as _cnt_sps, SUM(_cost)*-1 AS _sum_sps FROM get_view_in WHERE _id = ' . $i . ' AND _type = ' . $t . ' AND _count < 0 AND _date >= "' . $_GET['ot'] . '" AND _date < "' . $next_day_date . '") _sps
                 INNER JOIN (SELECT string_name AS string_name2, string_type AS string_type2, SUM(_count) AS _cnt_out, SUM(_cost) AS _sum_out FROM get_view_out WHERE _id = ' . $i . ' AND _type = ' . $t . '  AND _date >= "'.$_GET['ot'].'" AND _date < "' . $next_day_date . '") _out';
...
$result = $conn('SELECT SUM(_cost) as _cost FROM get_view_in WHERE _count > 0 AND _date >= "' . $_GET['ot'] . '" AND _date < "' . $next_day_date . '"');
foreach ($result as $row) {
    $tr(new HTEL('th colspan=2 &=text-align:right;/[0]', $row['_cost']));
    $sum_result -= $row['_cost'];
}

$result = $conn('SELECT SUM(_count)*-1 as _count FROM get_view_in WHERE _count < 0 AND _date >= "' . $_GET['ot'] . '" AND _date < "' . $next_day_date . '"');
foreach ($result as $row) {
    $tr(new HTEL('th &=text-align:center;/[0]', $row['_count']));
}

$result = $conn('SELECT SUM(_cost) as _cost FROM get_view_out WHERE _date >= "' . $_GET['ot'] . '" AND _date < "' . $next_day_date . '"');
foreach ($result as $row) {
